...
window.Chart = function(context, options){
	...
	var Tooltip = function(ctx, areaObj, data, type) {
		...
		this.render = function(x,y) {
			...
			//if(this.x != x || this.y != y) {
				...
				var tHeight=options.tooltips.padding.top+options.tooltips.padding.bottom;
				if(posY + tHeight > this.ctx.canvas.height) {
					posY -= tHeight;
				}
				this.ctx.fillStyle = options.tooltips.background;
				this.ctx.fillRect(posX, posY, rectWidth, tHeight);
				if(options.tooltips.border.width > 0) {
					...
					this.ctx.strokeRect(posX, posY, rectWidth, tHeight);
				}
				...
				this.ctx.fillText(tpl, posX+rectWidth/2, posY+tHeight/2);
				this.x = x;
				this.y = y;
			//}
		}
	...
	function getPosition(e) {
		...
		var yPosition = 0;
		while(e) {
			xPosition += ( e.clientLeft- e.scrollLeft );
			yPosition += ( e.clientTop- e.scrollTop );
			e = e.offsetParent;
		}
		...
		if(chart.tooltips.length > 0) {
			...
			for(var i in chart.tooltips) {
				var position = getPosition(context.canvas),
					mx = (e.offsetX)-position.x,
					my = (e.offsetY)-position.y;
